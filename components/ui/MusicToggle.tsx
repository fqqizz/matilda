...
export const MusicToggle = () => {
  const { isMobileMenuOpen, isCartOpen, isSearchOpen } = useStore();
  ...
  // Hide button when mobile menu, cart drawer, or search modal is open
  const isHidden = isMobileMenuOpen || isCartOpen || isSearchOpen;
      ...
      <AnimatePresence>
        {!isHidden && (
          <motion.div
            ...
            className="fixed bottom-4 left-4 sm:bottom-6 sm:left-6 z-30 font-sans select-none"
          >
            <button
              onClick={toggleSound}
              aria-label={isPlaying ? "Mute ambient background music" : "Play ambient MATILDA music"}
              className={`group flex items-center gap-2.5 px-3.5 py-2 rounded-full border transition-all duration-300 shadow-luxury backdrop-blur-md min-h-[40px] touch-manipulation ${
                isPlaying
                  ? "bg-[#1A0205]/95 border-[#C8A15A]/60 text-[#E4C98A]"
                  : "bg-[#1A0205]/85 border-[#3A080C] text-[#EFE3D2]/70 hover:text-[#E4C98A] hover:border-[#C8A15A]/40 active:scale-95"
              }`}
            >
